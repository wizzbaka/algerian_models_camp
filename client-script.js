...
async function loadModels() {
    ...
    try {
        ...
        if (models && models.length > 0) {
            ...
            // Stocker les modèles globalement (pour setupPaymentInfo)
...
function setupPaymentInfo() {
    ...
    function updatePriceDisplay() {
        ...
        if (selectedModelId) {
            ...
            if (selectedModel) {
                ...
                priceDifferenceElement.style.display = 'none';
                console.warn('⚠️ Modèle sélectionné non trouvé dans allModels');
            }
        } else {
    ...
    // Fonction pour charger et stocker tous les modèles
    async function loadAllModels() {
        try {
            ...
            if (models) {
                allModels = models;
                console.log(`✅ ${allModels.length} modèles chargés pour référence de prix`);
                
                // Initialiser les prix négociés
    ...
    if (modelSelect) {
        modelSelect.addEventListener('change', () => {
            ...
            // Mettre à jour le prix négocié si le modèle a déjà été ajusté
            const selectedModelId = modelSelect.value;
